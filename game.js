...
const MIN_PORTRAIT_HEIGHT = 1040;
...
const PHONE_LANDSCAPE_KEYBOARD_BREAKPOINT = 960;
...
class BrainstormGame {
  constructor(canvas, shell, answerProxy) {
    ...
    this.disableNativeTextInput();
    this.backgroundRain = this.createBackgroundRain();
  ...
  setupUi() {
    const menuCenterY = this.getMenuCenterY();
    this.playButton = rect(WIDTH / 2 - 115, menuCenterY + 225, 230, 62);
  ...
  getMenuCenterY() {
    if (!this.isPortraitLayout) return HEIGHT / 2;
    return Math.min(HEIGHT * 0.5, 690);
  }

  disableNativeTextInput() {
    if (!this.answerProxy) return;

    this.answerProxy.value = "";
    this.answerProxy.readOnly = true;
    this.answerProxy.setAttribute("inputmode", "none");
  }

  configureViewport() {
    const viewport = getViewportSize();
    const isPortrait = viewport.width <= PHONE_BREAKPOINT && viewport.height > viewport.width;
    const nextWidth = isPortrait ? PORTRAIT_WIDTH : LANDSCAPE_WIDTH;
    const nextHeight = isPortrait
      ? clamp(Math.round(nextWidth * viewport.height / viewport.width), MIN_PORTRAIT_HEIGHT, MAX_PORTRAIT_HEIGHT)
      : LANDSCAPE_HEIGHT;
    ...
    if (this.shell) {
      this.shell.dataset.layout = isPortrait ? "portrait" : "landscape";
    }
  }
  ...
  handleViewportChange() {
    ...
    if (oldWidth !== WIDTH || oldHeight !== HEIGHT) {
      this.resizeRaindrops(oldWidth, oldHeight);
      this.backgroundRain = this.createBackgroundRain();
    }
    this.setupUi();
  }

  resizeRaindrops(oldWidth, oldHeight) {
    if (!this.raindrops || oldWidth <= 0 || oldHeight <= 0) return;

    const scaleX = WIDTH / oldWidth;
    const scaleY = HEIGHT / oldHeight;
    this.raindrops.forEach((drop) => {
      drop.x = clamp(drop.x * scaleX, RAINDROP_SIZE / 2 + 30, WIDTH - RAINDROP_SIZE / 2 - 30);
      drop.y *= scaleY;
    });
  }

  createBackgroundRain() {
    const count = this.isPortraitLayout ? 72 : 42;
    return Array.from({ length: count }, () => ({
      x: Math.random() * WIDTH,
      y: Math.random() * HEIGHT,
      length: randomFloat(this.isPortraitLayout ? 22 : 16, this.isPortraitLayout ? 48 : 36),
      speed: randomFloat(this.isPortraitLayout ? 190 : 140, this.isPortraitLayout ? 320 : 250),
      slant: randomFloat(-11, -5),
      width: randomFloat(0.9, 1.8),
      alpha: randomFloat(0.14, this.isPortraitLayout ? 0.36 : 0.42),
      phase: Math.random() * Math.PI * 2,
      sway: randomFloat(0, 8),
      wind: randomFloat(-8, 10)
    }));
  }
  ...
  updateVirtualKeyboardMode() {
    ...
    this.virtualKeyboardEnabled = this.isPortraitLayout || (coarsePointer && viewport.width <= PHONE_LANDSCAPE_KEYBOARD_BREAKPOINT);
    this.updateLayout();
  }

  updateLayout() {
    const marginX = this.isPortraitLayout ? 30 : 130;
    this.layoutVirtualKeyboard();

    const keyboardTop = this.keyboardButtons.length
      ? Math.min(...this.keyboardButtons.map(({ area }) => area.y))
      : HEIGHT;
    const inputY = this.virtualKeyboardEnabled
      ? keyboardTop - (this.isPortraitLayout ? 84 : 74)
      : HEIGHT - (this.isPortraitLayout ? 96 : 86);
    this.inputBox = rect(marginX, inputY, WIDTH - marginX * 2, 62);
  }

  layoutVirtualKeyboard() {
    this.keyboardButtons = [];
    if (!this.virtualKeyboardEnabled) return;

    const marginX = this.isPortraitLayout ? 30 : 260;
    const gap = this.isPortraitLayout ? 12 : 10;
    const keyH = this.isPortraitLayout ? 80 : 42;
    const bottomMargin = this.isPortraitLayout ? 26 : 24;
    const keyW = (WIDTH - marginX * 2 - gap * 3) / 4;
    const startY = HEIGHT - bottomMargin - (keyH * 4 + gap * 3);
    const rows = [
      ["7", "8", "9", "Back"],
      ["4", "5", "6", "-"],
      ["1", "2", "3", "0"]
    ...
    rows.forEach((row, rowIndex) => {
      row.forEach((label, colIndex) => {
        this.keyboardButtons.push({
          label,
          area: rect(marginX + colIndex * (keyW + gap), startY + rowIndex * (keyH + gap), keyW, keyH)
        });
      });
    });
    ...
    this.keyboardButtons.push({
      label: "Enter",
      area: rect(marginX, startY + 3 * (keyH + gap), WIDTH - marginX * 2, keyH)
    });
  }
  ...
  bindEvents() {
    ...
    if (this.answerProxy) {
      this.answerProxy.addEventListener("focus", () => {
        this.answerProxy.blur();
      });
    }
  }
  ...
  drawBackground() {
    ...
    gradient.addColorStop(0.48, "#254f78");
  ...
  drawBackgroundRain() {
    ...
    ctx.save();
    ctx.lineCap = "round";
    this.backgroundRain.forEach((streak) => {
      const loopHeight = HEIGHT + streak.length + travelPad;
      const y = (streak.y + now * streak.speed) % loopHeight - streak.length - travelPad / 2;
      const drift = now * streak.wind + Math.sin(now * 0.8 + streak.phase) * streak.sway;
      const x = wrap(streak.x + drift, -travelPad, WIDTH + travelPad);

      ctx.beginPath();
      ctx.strokeStyle = `rgba(214, 244, 250, ${streak.alpha})`;
      ctx.lineWidth = streak.width;
      ctx.moveTo(x, y);
      ctx.lineTo(x + streak.slant, y + streak.length);
      ctx.stroke();
    });
    ctx.restore();
  }
