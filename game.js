...
class BrainstormGame {
  ...
  configureViewport() {
    ...
    const isPortrait = viewport.width <= NATIVE_KEYBOARD_BREAKPOINT && viewport.height > viewport.width;
  ...
  updateVirtualKeyboardMode() {
    ...
    this.nativeKeyboardEnabled = coarsePointer || viewport.width <= NATIVE_KEYBOARD_BREAKPOINT;
    this.updateLayout();
  }

  updateLayout() {
    const marginX = this.isPortraitLayout ? 30 : 130;
    const inputY = HEIGHT - (this.isPortraitLayout ? 96 : 86);
